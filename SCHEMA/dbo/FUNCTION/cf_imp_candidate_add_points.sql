CREATE OR REPLACE FUNCTION dbo.cf_imp_candidate_add_points(_user_id integer, _f_house uuid, _route_id uuid) RETURNS integer
    LANGUAGE plpgsql
    AS $$
DECLARE
	_result integer;
...
ALTER FUNCTION dbo.cf_imp_candidate_add_points(_user_id integer, _f_house uuid, _route_id uuid) OWNER TO mobnius;

COMMENT ON FUNCTION dbo.cf_imp_candidate_add_points(_user_id integer, _f_house uuid, _route_id uuid) IS 'ДОбавление точек маршрута для кандидата';
