CREATE OR REPLACE FUNCTION dbo.cf_mui_cs_answer(_fn_user integer, _c_version text) RETURNS TABLE(id bigint, c_text text, f_question integer, f_next_question integer, c_action text, n_order integer, b_disabled boolean, dx_created timestamp with time zone, sn_delete boolean, c_color text, f_role integer)
