...
BEGIN
	...
	-- создать маршрут
	insert into core.cd_routes(id, f_type, c_number, d_date, d_date_start, d_date_end, b_extended, c_notice, n_order, f_house)
	values (_route_id, _route_type_id, _route_number, now(), _date_activate, _date_limit, false, concat('M', _route_type_id, '-', _user_id, '-', date_part('month', now()), date_part('day', now())), _route_type_id, _f_house);
