...
CREATE OR REPLACE FUNCTION dbo.cf_imp_points(_user_id integer, _f_house uuid, _route_id uuid, _appartament_start integer, _appartament_end integer) RETURNS integer
    LANGUAGE plpgsql
    AS $$
DECLARE
	_result integer;
BEGIN
	...
	_result = (select count(*) from dbo.cs_appartament where f_main_user = _user_id and f_house = _f_house and n_number between _appartament_start and _appartament_end);
	RETURN _result;
END
$$;

ALTER FUNCTION dbo.cf_imp_points(_user_id integer, _f_house uuid, _route_id uuid, _appartament_start integer, _appartament_end integer) OWNER TO mobnius;

COMMENT ON FUNCTION dbo.cf_imp_points(_user_id integer, _f_house uuid, _route_id uuid, _appartament_start integer, _appartament_end integer) IS 'Импорт определенных точек маршрута';
