CREATE OR REPLACE FUNCTION dbo.cf_imp_candidate_points_vv(_user_id integer, _f_house uuid, _route_id uuid) RETURNS integer
    LANGUAGE plpgsql
    AS $$
DECLARE
	_result integer;
...
BEGIN
	...
	from dbo.msv_appartament_by_vv as a
	...
	_result = (select count(*) from dbo.msv_appartament_by_vv as a where a.f_subdivision = _sub_division and a.f_house = _f_house);
	RETURN _result;
END
$$;

ALTER FUNCTION dbo.cf_imp_candidate_points_vv(_user_id integer, _f_house uuid, _route_id uuid) OWNER TO mobnius;

COMMENT ON FUNCTION dbo.cf_imp_candidate_points_vv(_user_id integer, _f_house uuid, _route_id uuid) IS 'Импорт точек маршрута для ВВ';
