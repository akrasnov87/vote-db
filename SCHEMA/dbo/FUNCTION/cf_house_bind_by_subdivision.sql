CREATE OR REPLACE FUNCTION dbo.cf_house_bind_by_subdivision(_subdivision integer) RETURNS TABLE(id uuid, f_street uuid, c_street_name text, c_street_type text, c_house_num text, c_build_num text, n_uik integer, f_subdivision integer, n_all_appartament_count bigint, n_all_appartament_min integer, n_all_appartament_max integer, f_users json, b_disabled boolean, street_b_disabled boolean)
    LANGUAGE plpgsql
    AS $$
BEGIN
	return query 
