   ...
   FROM ( SELECT (t.table_name)::character varying AS table_name,
        ...
        UNION
         SELECT (t.table_name)::character varying AS table_name,
          ...
          WHERE ((t.table_type)::text = 'VIEW'::text)
        UNION
         SELECT (r.routine_name)::character varying AS table_name,
            ...
            r.routine_catalog AS table_catalog
           FROM ((information_schema.routines r
             LEFT JOIN pg_proc pgp ON ((pgp.proname = (r.routine_name)::name)))
             LEFT JOIN pg_description pgd ON ((pgd.objoid = pgp.oid)))) table1
  WHERE (((table1.table_schema)::text <> 'pg_catalog'::text) AND ((table1.table_schema)::text <> 'information_schema'::text) AND ((table1.table_schema)::text <> 'private'::text));
